  ...
  return (
    <section className="monitor-page">
      <div className="monitor-hero">
        ...
        <div><small>{status.runtime.enabled ? 'Agendamento ativo' : 'Agendamento desativado'}</small><h2>{status.runtime.running ? 'Varredura em andamento' : 'Monitor oficial em espera'}</h2><p>{status.runtime.running ? `${status.runtime.phase === 'analysis' ? 'Analisando documentos' : 'Consultando fontes'}${status.runtime.currentSource ? ` — ${status.runtime.currentSource}` : ''}` : `Próximo ciclo: ${dateTime(status.nextRunAt)} · intervalo de ${status.runtime.intervalMinutes} minutos`}</p></div>
        <div className="monitor-actions"><button disabled={status.runtime.running} onClick={() => start(false)}><RefreshCw size={16} />Só descobrir</button><button className="primary" disabled={status.runtime.running} onClick={() => start(true)}>{status.runtime.running ? <LoaderCircle className="spinning" size={16} /> : <Play size={16} />}Varrer e analisar</button></div>
      </div>
