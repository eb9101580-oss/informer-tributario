  ...
  return (
    <section className="monitor-page">
      <div className="monitor-hero">
        ...
        <div><small>{hosted ? 'Agendamento hospedado ativo' : status.runtime.enabled ? 'Agendamento local ativo' : 'Agendamento desativado'}</small><h2>{status.runtime.running ? 'Varredura em andamento' : 'Monitor oficial em espera'}</h2><p>{status.runtime.running ? `${status.runtime.phase === 'analysis' ? 'Analisando documentos' : 'Consultando fontes'}${status.runtime.currentSource ? ` — ${status.runtime.currentSource}` : ''}` : hosted ? 'O GitHub executa a coleta e a análise automaticamente a cada 6 horas.' : `Próximo ciclo: ${dateTime(status.nextRunAt)} · intervalo de ${status.runtime.intervalMinutes} minutos`}</p></div>
