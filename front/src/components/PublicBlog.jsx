  ...
  return (
    <div className="public-blog">
      ...
      <main className="public-blog__main">
        ...
        {loading && !alerts.length ? <div className="public-blog__loading"><span className="loading-spinner" />Carregando o feed...</div> : <section className="public-blog__feed"><div className="public-blog__feed-heading"><h2>Últimos 14 dias</h2><span>{feed.length} itens</span></div><div className="alerts-list">{feed.map((alert) => <AlertCard key={alert.id} alert={alert} onOpen={setSelected} />)}</div>{!feed.length && <p className="public-blog__empty">Nenhuma publicação relevante dos últimos 14 dias foi analisada ainda.</p>}</section>}
